...
public class Pistol : MonoBehaviour, IWeapon
{
    ...
    private float spreadMagnitude = 0.05f; // A pistol typically has minimal spread
    private float spreadAngle = 1f; // 1 degree of spread for a pistol seems fair
    ...
    public void Shoot()
    {
        if (BulletTrail == null)
        {
            Debug.Log("BulletTrail prefab is null");
        }
        if (impactEffect == null)
        {
            Debug.Log("impactEffect spawn point is null");
        }
        if (muzzleFlashes == null)
        {
            Debug.Log("muzzleFlashes spawn point is null");
        }
        ...
        if (Time.time >= nextTimeToFire)
        {
            ...
            // Determine the direction of the spread
